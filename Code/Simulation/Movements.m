    ...
    methods(Static)%Class use static methods
        ...
        function moveikcon(robot,linkNum,T,steps,Obstacle) 
            ...
            for i = 1:steps
                ...
                Collision.isCollision(CollidB, centerPoint, radii, Obstacle);
                Collision.isCollision(lightC, centerPoint, radii, Obstacle);
                drawnow();
            end
        end
...
%               x(6,i) = 0;
            ...
            for i = 1:steps-1
                ...
                newPose1 = robot.model.fkine(qMatrix1(i,:));
                  ...
                  cond3.move(newPose1*trotx(pi)*transl(0.05,-0.10,-0.12));
                drawnow();
            end
...
%             end
        ...
        %% Resolve Motion Rate Control 3  (for coffee)
        function rmrc3(robot, linkNum, finalPos, obj, steps)
            deltaT = 0.05;                                        % Discrete time step
            x = zeros(3,steps);
            s = lspb(0,1,steps);                                  % Create interpolation scalar
            initialPos = robot.model.fkine(robot.model.getpos());
...
%               x(6,i) = 0;
        ...
        %% Resolve Motion Rate Control (not currently in use) -----------------------------------
